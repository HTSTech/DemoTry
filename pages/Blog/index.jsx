...
export default function Home() {
  ...
  useEffect(() => {
    ...
    getServerData(selectedCategory, pageNumber, search, false);
  }, []);

  function getServerData(_selectedCategory, _pageNumber, _search, isMerge) {
    setIsLoading(true);
    axios.get('https://ticklelife-backend.de.r.appspot.com/blog/DemoTry/'+_selectedCategory+'/'+_pageNumber+"?search="+_search).then((resp) => {
  ...
  function loadMode() {
    ...
    getServerData(selectedCategory, _pageNumber, search, true);
  }
    ...
    <div>
      ...
      <div id="BlogID">
        <div style={{minHeight: '500px'}}>
          ...
          <div style={{display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center", padding: "50px 10px", gap: "10px"}}>
            <div>
              <input placeholder="Search" type="text" onChange={(e) => {setSearch(e.target.value)}} style={{background: '#FFFFFF', boxShadow: '0px 1px 2px rgba(16, 24, 40, 0.05)', borderRadius: '8px', padding: '5px', fontFamily: '\'Inter\'', fontStyle: 'normal', fontWeight: '400', fontSize: '16px', lineHeight: '24px', color: '#667085', outline: 'none', border: "none"}}/>
              <button style={{padding: '8px', borderRadius: '8px', border: '0', background: 'rgb(241, 249, 244)', cursor: "pointer"}} onClick={() => {getServerData(selectedCategory, 1, search, false)}}>Search</button>
            </div>
          </div>
